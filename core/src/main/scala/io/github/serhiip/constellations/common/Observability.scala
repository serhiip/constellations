...
private[constellations] object Observability:
  ...
  extension [F[_]](tracer: Tracer[F]) inline def span(parts: String*) = tracer.span(spanName(namePrefix +: parts*))
  ...
  extension [F[_]: StructuredLogger](span: Span[F])
    ...
    inline def loggedR[A](action: StructuredLogger[F] => Resource[F, A]): Resource[F, A] = action(populateContext(StructuredLogger[F], span.context))
