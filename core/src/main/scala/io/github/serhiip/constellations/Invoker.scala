...
object Invoker:
  ...
  private def observed[F[_]: Monad: Tracer: StructuredLogger, T](delegate: Invoker[F, T]): Invoker[F, T] = new Invoker[F, T]:
  ...
  def retrying[F[_]: StructuredLogger: Temporal, T](maxDelay: FiniteDuration, initialDelay: FiniteDuration, delegate: Invoker[F, T]): Invoker[F, T] = new:

    def logError(err: Throwable, details: RetryDetails): F[Unit] =
      val logger = StructuredLogger[F]
      details.nextStepIfUnsuccessful match
        case RetryDetails.NextStep.DelayAndRetry(nextDelay: FiniteDuration) => logger.trace(s"Invoker retry after ${details.retriesSoFar} attemps")
        case RetryDetails.NextStep.GiveUp                                   => logger.error(err)(s"Giving up after ${details.retriesSoFar} retries")

    override def generate(prompt: NEC[Message]): F[T] = retryingOnErrors(delegate.generate(prompt))(
